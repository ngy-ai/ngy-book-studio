...
- [x] 新建与编辑
  ...
  - [x] EPUB/MOBI/AZW/AZW3/Word 结构化图书的整本后台翻译：以
    ...
    整行标记文本、代码运算符）由宿主 `markup::looks_like_source_code` 与阅读器
    `translations.js` 按同一套规则跳过，任务列表里不再出现这些块
  - [x] 阅读器翻译显示：打开章节时按当前单元读取与当前版本、对话模型一致的译文并经
    session/generation 门控注入；默认“译文”只显示译文、隐藏原文，点击段落切换该段的
    “译文/双语”，系统配置可全局选择“双语/原文/译文”。阅读窗口工具栏另有同一套三态
    ...
    **图书的选择优先于全局默认**；全局变化只影响没有自己选择的图书，切换旁的“跟随全局”
    ...
    `data-moye-translation` 并从笔记文本索引与选区文本中排除，原文文本节点始终
    保留在 `body`，因此划线、人工/AI 想法与版本校验不受影响。选中译文本身也可以划线、
    写想法或“AI解释”：`moyeTranslations.originalRange()` 把选区命中的译文叶子映射回
...
- [x] 搜索与派生索引
  ...
  - [x] 翻译任务的详情面板提供“文本块明细”：按任务游标顺序列出每个文本块的状态
    ...
    进度；状态由持久游标 `completed` 推导，标记为“处理中”的只有游标当前指向的一个块。
    明细只读，仅覆盖当前来源的文本块，显示数量与任务进度使用同一份快照
  - [x] 后台任务默认不自动运行：新导入、创建或保存编辑仍持久创建
    ...
    不打断或改变此前已排队、正在运行的任务
  - [x] 后台任务并发与任务间隔可配置（AI 设置“后台任务”：并发 1–8 默认 1，任务间隔
    0–60000 毫秒默认 10）；保存后对后续任务立即生效，不打断正在运行的任务
...
- [x] 2026-09-11 现场 1269 号文本块修复（日志只有一句 `http_timeout`：请求 120 秒后被
  ...
  `translation_block_saved` 耗时，`MOYE_DUMP_TRANSLATION_RAW` 现在也覆盖流失败。回归：
